...
class TestModelicaCalibrator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.example_cal_dir = os.path.normpath(self.example_dir + "//test_calibration")
        self.example_mat_file = os.path.normpath(self.example_dir +
                                                 "//Modelica//ref_result.mat")

        # As all test rely on goals, tuner_parameter and a
        # calibration class, we will always create them here
        self.tuner_paras = data_types.TunerParas(["heatConv_a", "heatConv_b", "C", "m_flow_2"],
                                                 [130, 220, 5000, 0.04],
                                                 [(100, 500), (100, 500),
                                                  (200, 10000), (0.005, 0.05)])
        mtd = data_types.MeasTargetData(self.example_mat_file)
        std = data_types.SimTargetData(self.example_mat_file)
        cols = ["heater.heatPorts[1].T", "heater1.heatPorts[1].T"]
        self.goals = data_types.Goals(mtd, std, meas_columns=cols,
                                      sim_columns=cols, weightings=[0.7, 0.3])
        self.calibration_class = data_types.CalibrationClass("Device On", 0, 3600,
                                                             goals=self.goals,
                                                             tuner_paras=self.tuner_paras)
        ...
        packages = [os.path.normpath(self.example_dir + "//Modelica//AixCalTest//package.mo")]
        self.model_name = "AixCalTest.TestModel"
        try:
            self.dym_api = DymolaAPI(self.example_cal_dir, self.model_name, packages)
